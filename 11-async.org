...
- fs with callbacks
- Using promises
  - Not about how to create promises (we don't need to know that now, just like we don't need to know how to create console.log() or other built-in functions from scratch)
  - .then()
  - .catch()
  - Chaining
  - fs with promises
- async/await rules
  - If a function is defined with "async", then it must be called with "await"
  - If a function has "await" inside it, then it must be defined with "async"
