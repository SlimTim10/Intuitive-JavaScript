...
*** map()
Probably the most commonly used of the higher-order array methods is ~map()~. It takes in a function and returns a new arrays after having applied that function to each element. Because the function we provide to ~map()~ is meant to operate on each individual element of the array, we must define it as a function that takes one argument and returns a new value.
...
- start with an array,
- and end up with a new array of less than or equal size as the original, without changing the elements.
...
** Converting between arrays and strings 						   :noexport:
